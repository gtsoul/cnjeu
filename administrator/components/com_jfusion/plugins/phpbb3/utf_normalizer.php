...
class utf_normalizer_phpbb {
    ...
    function cleanup(&$str) {
        ...
        if (!isset($GLOBALS['utf_nfc_qc'])) {
            require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . 'phpbb3' . DS . 'utf' . DS . 'utf_nfc_qc.php';
        }
        if (!isset($GLOBALS['utf_canonical_decomp'])) {
            require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . 'phpbb3' . DS . 'utf' . DS . 'utf_canonical_decomp.php';
        }
        ...
        $str = strtr($str, "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x0B\x0C\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F", "\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF");
    ...
    function nfkc(&$str) {
        ...
        if (!isset($GLOBALS['utf_compatibility_decomp'])) {
            require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . 'phpbb3' . DS . 'utf' . DS . 'utf_compatibility_decomp.php';
        }
    ...
    function recompose($str, $pos, $len, &$qc, &$decomp_map) {
        ...
        if (!isset($utf_jamo_index, $utf_jamo_type, $utf_combining_class)) {
            require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . 'phpbb3' . DS . 'utf' . DS . 'utf_normalizer_common.php';
        }
        // Load the canonical composition table
        if (!isset($utf_canonical_comp)) {
            require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . 'phpbb3' . DS . 'utf' . DS . 'utf_canonical_comp.php';
        }
        ...
        $extra_check = array("\xED" => 1, "\xEF" => 1, "\xC0" => 1, "\xC1" => 1, "\xE0" => 1, "\xF0" => 1, "\xF4" => 1, "\xF5" => 1, "\xF6" => 1, "\xF7" => 1, "\xF8" => 1, "\xF9" => 1, "\xFA" => 1, "\xFB" => 1, "\xFC" => 1, "\xFD" => 1, "\xFE" => 1, "\xFF" => 1);
        $utf_validation_mask = array(2 => "\xE0\xC0", 3 => "\xF0\xC0\xC0", 4 => "\xF8\xC0\xC0\xC0");
        $utf_validation_check = array(2 => "\xC0\x80", 3 => "\xE0\x80\x80", 4 => "\xF0\x80\x80\x80");
        // Main loop
        do {
