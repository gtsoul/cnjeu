...
function updateJavaScript(plugin,field, value) {
	...
	tdElem.firstChild.href = "javascript: changesetting('"+field+"','"+value+"','"+plugin+"')";
...
function copyplugin(jname) {
    ...
    if(new_jname) {
     ...
     // this code will send a data object via a GET request and alert the retrieved data.
        var jsonRequest = new Request.JSON({url: url , onSuccess: function(results){
            ...
            if(results.status ==  'success') {
                ...
                //report success
                alert('<?php echo JText::_('COPY') .' ' . JText::_('SUCCESS');?>');
...
function addRow(new_jname, rowhtml){
    var tbody = document.getElementById("sortables").getElementsByTagName("TBODY")[0];
    var row = document.createElement("TR");
    row.setAttribute("id",new_jname);
...
function deleteplugin(jname) {
    ...
    if(confirmdelete) {
    	//update the database
        var url = '<?php echo JURI::root() . 'administrator/index.php'; ?>';    	
    	sync_vars = 'option=com_jfusion&task=uninstallplugin&tmpl=component&jname='+jname;
               	    	

        var ajaxsync = new Request.HTML({ url: url, method: 'get'});
        ajaxsync.send(sync_vars);
        var el = document.getElementById(jname);
...
var mySortableTable;
  ...
  window.addEvent('domready',function(){
	$('installSVN').set('send', {	onComplete: function(JSONobject) {
	...
	$('installURL').set('send', {	onComplete: function(JSONobject) {
	 	var response = JSON.decode(JSONobject);
		if (response.overwrite != 1 && response.status == true) {
			addRow(response.jname, response.rowhtml);
		}
	...
	$('installDIR').set('send', {	onComplete: function(JSONobject) {
		var response = JSON.decode(JSONobject);
		if (response.overwrite != 1 && response.status == true) {
			addRow(response.jname, response.rowhtml);
		}
...
<input type="hidden" name="option" value="com_jfusion" />
<input type="hidden" name="task" value="installplugin2" />
...
<table class="adminform"><tr><td>
    ...
    <input type="submit"  value="<?php echo JText::_('INSTALL'); ?>"/><div id="spinnerDIR"></div>
    </td></tr></table>
</td></tr></table>
</form>
...
<table class="adminform"><tr><td>
    ...
    <input type="submit"   value="<?php echo JText::_('INSTALL'); ?>"/><div id="spinnerURL"></div>
    </td></tr></table>
</td></tr></table>
</form>
