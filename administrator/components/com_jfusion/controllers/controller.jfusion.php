    ...
    function wizardresult()
    {
        ...
        if ($jname && $post) {
            ...
            if ($params) {
                ...
                JFusionFunction::saveParameters($jname, $params, true);
   ...
   /**
    ...
    function syncresume()
    {
        ...
        if ($db->loadResult()) {
            ...
            include_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'models' . DS . 'model.usersync.php';
    ...
    function installplugins()
    {
		...
		foreach ($jfusionplugins as $plugin) {
                //install updates
                $packagename = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'packages' . DS . 'jfusion_' . $plugin . '.zip';
                $model = new JFusionModelInstaller();
    ...
    function plugincopy()
    {
        ...
        if ($jname && $new_jname) {
            ...
            //get description
            $plugin_xml = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' .DS.$jname.DS.'jfusion.xml';
            ...
            $response['new_jname'] =  $new_jname;
            $view = $this->getView('plugindisplay','html');
            $response['rowhtml'] = $view->generateRowHTML($view->initRecord($new_jname));            
            ...
            $response['message'] =  JText::_('NONE_SELECTED');        	
        }
    }
    ...
    function enableplugins()
    {
        ...
        $db->setQuery($query);
        if ($db->loadResult()) {
            JFusionFunction::changePluginStatus('joomla','authentication',0);
            JFusionFunction::changePluginStatus('joomla','user',0);
            JFusionFunction::changePluginStatus('jfusion','authentication',1);
            JFusionFunction::changePluginStatus('jfusion','user',1);
        } else {
    ...
    function disableplugins()
    {
        ...
        JFusionFunction::changePluginStatus('joomla','authentication',1);
        JFusionFunction::changePluginStatus('joomla','user',1);
        JFusionFunction::changePluginStatus('jfusion','authentication',0);
        JFusionFunction::changePluginStatus('jfusion','user',0);
        JRequest::setVar('view', 'cpanel');
        $this->display();
    }
