...
class JFusionFactory
{
    ...
    function &getHelper($jname)
    {
        ...
        if (!isset($helper_instances[$jname])) {
            $filename = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jfusion' . DS . 'plugins' . DS . $jname . DS . 'helper.php';
            if (file_exists($filename)) {
                //load the plugin class itself
                include_once $filename;
     ...
     */
    function getPluginNodeId($jname) {
        $params = JFusionFactory::getParams($jname);
     ...
     */
    function getPluginNameFromNodeId($jnode_id) {
        $result = '';
        //$jid = $jnode_id;
     ...
     */
    function &getCodeParser($type = 'bbcode') {
