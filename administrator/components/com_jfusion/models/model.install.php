...
//        $this->parent = & JInstaller::getInstance();
    ...
    function install($dir = null, &$result)
    {
        ...
        if ($dir && JFolder::exists($dir)) {
            ...
            $msg = JText::_('INSTALL_INVALID_PATH');
    ...
    function uninstall($jname)
    {
        ...
        if (!$jname || !JFolder::exists($dir)) {
            $msg = JText::_('UNINSTALL_ERROR_PATH');
            $this->parent->abort($msg);
            $result['message'] = $msg;
            $result['status'] = false;
            return $result;
        ...
        $manifest = $this->_getManifest($dir);
        if (is_null($manifest)) {
            $msg = JText::_('INSTALL_NOT_VALID_PLUGIN');
            $this->parent->abort($msg);
            $result['message'] = $msg;
            $result['status'] = false;
            return $result;
        ...
        if (!JFolder::delete($dir)) {
            $msg = JText::_('UNINSTALL_ERROR_DELETE');
            $this->parent->abort($msg);
            $result['message'] = $msg;
            $result['status'] = false;
            return $result;
    ...
    function copy($jname, $new_jname, $update = false)
    {
        ...
        $msg = JText::_('PLUGIN') . ' ' .$jname .' ' . JText::_('COPY') . ': ' . JText::_('SUCCESS');
        $result['message'] = $msg;
        $result['status'] = true;
        return $result;
    }
