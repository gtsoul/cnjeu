...
<form action="<?php echo JRoute::_('index.php?option=com_icagenda&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="event-form" class="form-validate" enctype="multipart/form-data">
	<div class="container">
		...
		<!-- iCagenda Header -->
		<header>
			<h1><?php echo empty($this->item->id) ? JText::_('COM_ICAGENDA_LEGEND_NEW_EVENT') : JText::sprintf('COM_ICAGENDA_LEGEND_EDIT_EVENT', $this->item->id); ?> <span>iCagenda</span></h1>
			<h2>
		...
		if(version_compare(JVERSION, '3.0', 'lt')) {
			...
			if (!$scriptFound) {
				...
				if (!JFactory::getApplication()->get('jquery')) {
		    		...
		    		$document->addScript(JURI::base().'components/com_icagenda/js/jquery.noconflict.js');
				}
			}
			// Google Maps api V3
			...
			JHTML::script('icmap.js', 'administrator/components/com_icagenda/add/js/');
			JHTML::script('icdates.js', 'administrator/components/com_icagenda/add/js/');
			JHTML::script('timepicker.js', 'administrator/components/com_icagenda/add/js/');
			JHTML::script('submitbutton.js', 'administrator/components/com_icagenda/views/event/');
...
//			$document->addScript('https://ajax.googleapis.com/ajax/libs/jqueryui/1.9.2/jquery-ui.min.js');
        	...
        	$document->addScript( JURI::base().'components/com_icagenda/add/js/icmap.js' );
			$document->addScript( JURI::base().'components/com_icagenda/add/js/icdates.js' );
			$document->addScript( JURI::base().'components/com_icagenda/add/js/timepicker.js' );
//			$document->addScript('/administrator/components/com_icagenda/views/event/submitbutton.js');
		}
