...
/**
 * Methods supporting a list of distinctions records.
 *
 * @package		Joomla.Administrator
 * @subpackage	com_cnj_jeux
 * @since		1.6
 */
class Cnj_jeuxModelDistinctions extends JModelList
{
	/**
	 * Constructor.
	 *
        ...
        protected function getListQuery()
	{
            ...
            if (!empty($titre)) {
                    if (stripos($titre, 'id:') === 0) {
                            $query->where('d.id_distinction = '.(int) substr($titre, 3));
                    } else {
        ...
        public function delete($ids)
	{
            ...
            foreach ($ids as $id) {
                $db = $this->getDbo();
                $db->setQuery(
                        'DELETE FROM cnj_distinction' .
                        ' WHERE id_distinction = ' . (int)$id
                );
                $db->query();
