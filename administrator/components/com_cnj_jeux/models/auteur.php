 ...
 * @since       1.6
 */
class Cnj_jeuxModelAuteur extends JModelAdmin
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cnj_jeux.auteur', 'auteur', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_cnj_jeux.edit.auteur.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
                        
                        $data->documents = $this->_getDocuments($data->id_auteur);
		}
	...
	private function _getDocuments($id)
	{
            ...
            $query->select($this->getState('list.select', 'd.id, d.nom, ad.ordre'));
