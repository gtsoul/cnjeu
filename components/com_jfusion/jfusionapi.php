...
class JFusionAPI {
	...
	private function retrieveKey()
	{
		...
		$FileData = $this->_raw('get','key', 'key', null, false);
		if ($this->error) {
			return false;
	...
	public function get($class, $task, $payload)
	{
    ...
    private function post($post=array())
    {
        ...
        if (!function_exists('curl_init')) {
        	$this->error[] = 'sorry cURL is needed for JFusionAPI';
        	return;
        }
        if ($this->sid) {
	...
	private function getOutput($input)
	{
		...
		if (!is_array($return)) {
			$this->error[] = 'error reading api output: '. $input;
			return false;
...
class JFusionAPI_Key extends JFusionAPIBase {
