...
class plgUserJfusion extends JPlugin
{
    ...
    function onLoginUser($user, $options)
    {
        ...
        if (!empty($master)) {
            ...
            // redirect with external link added 28/7/2010 HJW
            $params = & JFusionFactory::getParams('joomla_int');
            $allowredir = $params->get('allow_redirect_login', 0);
            $URLredir = $params->get('redirecturl_login', '');

            if ((!$mainframe->isAdmin())&& $allowredir && !empty($URLredir)) // only redirect if we are in the frontend and allowed and have an URL
            {
                $mainframe->redirect($URLredir); /* Redirect browser */
            }
            ob_end_clean();
